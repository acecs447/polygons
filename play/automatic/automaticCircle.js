  ...
  for(var x=0;x<GRID_SIZE;x++){
    for(var y=0;y<GRID_SIZE;y++){
      ...
      if(rand<(1-window.EMPTINESS)){
        ...
        if(inCircle(draggable.x, draggable.y) == true){
...
function inCircle(x, y){
  ...
  var radius = 300;
  var square_dist = Math.pow((centerX - x), 2) + Math.pow((centerY - y), 2);
    return square_dist <= (Math.pow(radius, 2) - 7000);
}
...
function step(){
  ...
  while (inCircle(spot.x, spot.y) == false) {
